...
io.on("connection", (socket) => {
    socket.on("emit_raffle_participants", ({raffleId}) => {
        io.emit(`updated-participants-raffle-${raffleId}`, raffles[raffleId]?.participants || []);
    });
    ...
    socket.on("join_raffle", ({raffleId, name, userId}) => {
        ...
        if (
            raffles[raffleId]?.participants?.find((participant) => participant.userId === userId)
        ) {
            ...
            raffles[raffleId].participants.push({name, userId, socketId: socket.id});
        }
